...
KeyboardItem::KeyboardItem()
    : stateFlags(NOT_PRESSED), previousState(NOT_PRESSED), pin(-1), counter(0),
      acceleration(0), repeatInterval(NO_REPEAT), notify{},
      callbackOnRelease{} {}

KeyboardItem::KeyboardItem(pinid_t pin, KeyCallbackFn callback,
                           uint8_t repeatInterval, bool keyLogicIsInverted)
    : notify{} {
...
KeyboardItem::KeyboardItem(pinid_t pin, SwitchListener *switchListener,
                           uint8_t repeatInterval, bool keyLogicIsInverted)
    : notify{} {
  this->pin = pin;
  this->repeatInterval = repeatInterval;
...
void KeyboardItem::onRelease(KeyCallbackFn cb) { this->callbackOnRelease = cb; }
...
void KeyboardItem::trigger(bool held) {
  ...
  if (isUsingListener())
    notify.listener->onPressed(pin, held);
  else
    notify.callback(pin, held);
}

void KeyboardItem::triggerRelease(bool held) {
  if (isUsingListener())
    notify.listener->onReleased(pin, held);
  else if (callbackOnRelease)
    callbackOnRelease(pin, held);
}
...
void KeyboardItem::checkAndTrigger(uint8_t buttonState) {
  if (notify.callback == nullptr && callbackOnRelease == nullptr)
    return;
  ...
  if (buttonState == HIGH) {
    ...
    } else if (getState() == BUTTON_HELD && repeatInterval != NO_REPEAT &&
...
void SwitchInput::initialiseInterrupt(IoAbstractionRef device,
                                      bool usePullUpSwitching) {
...
void SwitchInput::init(IoAbstractionRef device, SwitchInterruptMode mode,
  ...
  bitWrite(swFlags, SW_FLAG_ENCODER_IS_POLLING,
           (mode == SWITCHES_POLL_EVERYTHING));
  ...
  serlogF4(SER_IOA_INFO, "Switches initialized (pull-up, int, encPoll)",
           bitRead(swFlags, SW_FLAG_PULLUP_LOGIC),
           bitRead(swFlags, SW_FLAG_INTERRUPT_DRIVEN),
           bitRead(swFlags, SW_FLAG_ENCODER_IS_POLLING));
}

bool SwitchInput::addSwitch(pinid_t pin, KeyCallbackFn callback, uint8_t repeat,
                            bool invertLogic) {
  internalAddSwitch(pin, invertLogic);
  return keys.add(KeyboardItem(pin, callback, repeat, invertLogic));
}

bool SwitchInput::addSwitchListener(pinid_t pin, SwitchListener *listener,
                                    uint8_t repeat, bool invertLogic) {
  internalAddSwitch(pin, invertLogic);
...
bool SwitchInput::internalAddSwitch(pinid_t pin, bool invertLogic) {
  if (ioDevice == nullptr)
    initialise(internalDigitalIo(), true);
...
void SwitchInput::changeEncoderPrecision(uint8_t slot, uint16_t precision,
                                         uint16_t currentValue, bool rollover,
                                         int step) {
  if (slot < MAX_ROTARY_ENCODERS && encoder[slot] != nullptr) {
    encoder[slot]->changePrecision(precision, (int)currentValue, rollover,
                                   step);
  }
}
...
bool SwitchInput::runLoop() {
  ...
  for (bsize_t i = 0; i < keys.count(); ++i) {
    ...
    // we need to call into here again if we are debouncing or anything is
    // pressed.
  ...
  if (!needAnotherGo) {
    for (int i = 0; i < MAX_ROTARY_ENCODERS; ++i) {
      if (switches.encoder[i]) {
        switches.encoder[i]->checkAndTrigger();

        // we need to call into here again if we are debouncing
        needAnotherGo |= (switches.encoder[i]->isDebouncing);
      }
    }
  }
  ...
  if (incVal >= 0) {
    if (rollover) {
      ...
      if (currentReading > maximumValue)
        currentReading = currentReading - maximumValue - 1;
    } else {
...
HardwareRotaryEncoder::HardwareRotaryEncoder(
    ...
    HWAccelerationMode accelerationMode, EncoderType encoderType)
...
HardwareRotaryEncoder::HardwareRotaryEncoder(
    ...
    HWAccelerationMode accelerationMode, EncoderType encoderType)
    : RotaryEncoder(listener) {
  initialise(pinA, pinB, accelerationMode, encoderType);
}

void HardwareRotaryEncoder::initialise(pinid_t pinA, pinid_t pinB,
...
void checkRunLoopAndRepeat() {
  ...
  // instead of running constantly, we only run when there's a need to, eg
  // something is still in a debouncing state. Otherwise we wait for an
  // interrupt. switches.runLoop returns true when it needs to run again.
  if (switches.runLoop() && switches.isInterruptDriven()) {
    taskManager.scheduleOnce(20, [] { checkRunLoopAndRepeat(); });
  } else {
...
int HardwareRotaryEncoder::amountFromChange(unsigned long change) {
  if (change > 250000 || maximumValue < ONE_TURN_OF_ENCODER)
    return stepSize;
  ...
  if (accelerationMode == HWACCEL_NONE) {
    ...
    else if (change > 70000)
      return stepSize << 2;
    else if (change > 30000)
      return stepSize << 3;
    else
      return stepSize << 4;
...
void HardwareRotaryEncoder::encoderChanged() {
  ...
  // do some checking on debouncing
  ...
  if (encoderType == QUARTER_CYCLE) {
    if ((a != aLast) || (b != cleanFromB)) {
      Serial.println("One of the two changed since last time");
      aLast = a;
      if ((a != aLast) || (b != cleanFromB)) {
        cleanFromB = b;
...
void HardwareRotaryEncoder::handleChangeRaw(bool increase) {
  ...
  // get the amount of change and direction. Also keep a copy of the time until
  // later for acceleration purposes
  ...
  // direction changes within the reject change threshold would not result in an
  // encoder change, part of the debounce logic to prevent spurious updates.
  // Within this period direction must be the both now and previously.
  // serlogF4(SER_DEBUG, "delta ", deltaMillis, increase, lastDirectionUp);
  if (deltaMillis < REJECT_DIRECTION_CHANGE_THRESHOLD &&
      increase != lastDirectionUp)
    return;

  // now we make the change and register the last change direction (as we
  // accepted it)
  increment((int8_t)(increase ? amt : -amt));
  bitWrite(flags, LAST_ENCODER_DIRECTION_UP, increase);
}

EncoderUpDownButtons::EncoderUpDownButtons(pinid_t pinUp, pinid_t pinDown,
                                           EncoderCallbackFn callback,
                                           uint8_t speed)
    : RotaryEncoder(callback), upPin(pinUp), downPin(pinDown), backPin(-1),
      nextPin(-1), passThroughListener(nullptr), canRotate(false) {
...
EncoderUpDownButtons::EncoderUpDownButtons(pinid_t pinUp, pinid_t pinDown,
                                           ...
                                           uint8_t speed)
    : RotaryEncoder(listener), upPin(pinUp), downPin(pinDown), backPin(-1),
      nextPin(-1), passThroughListener(nullptr), canRotate(false) {
  switches.addSwitchListener(pinUp, this, speed);
  switches.addSwitchListener(pinDown, this, speed);
}
...
EncoderUpDownButtons::EncoderUpDownButtons(pinid_t pinUp, pinid_t pinDown,
                                           pinid_t backPin, pinid_t nextPin,
                                           SwitchListener *passThrough,
                                           EncoderCallbackFn callback,
                                           uint8_t speed)
    : RotaryEncoder(callback), upPin(pinUp), downPin(pinDown), backPin(backPin),
      nextPin(nextPin), passThroughListener(passThrough), canRotate(true) {
  switches.addSwitchListener(pinUp, this, speed);
  switches.addSwitchListener(pinDown, this, speed);
...
EncoderUpDownButtons::EncoderUpDownButtons(pinid_t pinUp, pinid_t pinDown,
                                           ...
                                           EncoderListener *listener,
                                           uint8_t speed)
    : RotaryEncoder(listener), upPin(pinUp), downPin(pinDown), backPin(backPin),
      nextPin(nextPin), passThroughListener(passThrough), canRotate(true) {
  switches.addSwitchListener(pinUp, this, speed);
  switches.addSwitchListener(pinDown, this, speed);
  switches.addSwitchListener(backPin, this, speed);
...
void setupUpDownButtonEncoder(pinid_t pinUp, pinid_t pinDown,
                              EncoderCallbackFn callback, int speed) {
...
void setupUpDownButtonEncoder(pinid_t pinUp, pinid_t pinDown,
                              EncoderListener *listener, int speed) {
  if (switches.getIoAbstraction() == nullptr)
    switches.init(internalDigitalIo(), SWITCHES_POLL_EVERYTHING, true);
...
void setupUpDownButtonEncoder(pinid_t pinUp, pinid_t pinDown, pinid_t pinLeft,
  ...
  auto *enc = new EncoderUpDownButtons(pinUp, pinDown, pinLeft, pinRight,
                                       passThroughListener, listener, speed);
  switches.setEncoder(enc);
}
void setupUpDownButtonEncoder(pinid_t pinUp, pinid_t pinDown, pinid_t pinLeft,
                              pinid_t pinRight,
                              SwitchListener *passThroughListener,
                              EncoderCallbackFn cb, int speed) {
  if (switches.getIoAbstraction() == nullptr)
    switches.init(internalDigitalIo(), SWITCHES_POLL_EVERYTHING, true);

  auto *enc = new EncoderUpDownButtons(pinUp, pinDown, pinLeft, pinRight,
                                       passThroughListener, cb, speed);
  switches.setEncoder(enc);
}
...
void setupRotaryEncoderWithInterrupt(pinid_t pinA, pinid_t pinB,
                                     EncoderCallbackFn callback,
                                     HWAccelerationMode accelerationMode,
                                     EncoderType encoderType) {
  if (switches.getIoAbstraction() == nullptr)
...
void setupRotaryEncoderWithInterrupt(pinid_t pinA, pinid_t pinB,
  ...
  switches.setEncoder(new HardwareRotaryEncoder(pinA, pinB, listener,
                                                accelerationMode, encoderType));
}
